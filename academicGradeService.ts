...
const emptyRecord = (
  ...
  pengajianSore: null,
  akhlak: '',
  absensi: { sakit: null, izin: null, alpha: null },
  ranking: null,
  catatan: '',
  keputusan: '',
  musrif: '',
...
export const academicGradeService = {
  get(
    ...
    try {
      ...
      return {
        ...emptyRecord(nis, tahunAjaran, semester, defaultMapels),
        ...parsed,
        nis,
        tahunAjaran,
        semester,
        ...
        madrasatulQuran: {
          kelancaran: parsed.madrasatulQuran?.kelancaran ?? null,
          makhroj: parsed.madrasatulQuran?.makhroj ?? null,
          tajwid: parsed.madrasatulQuran?.tajwid ?? null,
        },
        absensi: {
          sakit: parsed.absensi?.sakit ?? null,
          izin: parsed.absensi?.izin ?? null,
          alpha: parsed.absensi?.alpha ?? null,
        },
      };
    } catch {
      return emptyRecord(nis, tahunAjaran, semester, defaultMapels);
    }
  },

  save(record: AcademicGradeRecord): void {
    localStorage.setItem(key(record.tahunAjaran, record.semester, record.nis), JSON.stringify(record));
  },
};
